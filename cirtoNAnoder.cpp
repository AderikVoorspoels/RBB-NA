...
class cirtoNAnoder : public Colvar {
  bool components;
  bool pbc;
  bool RNA;
  bool FITTED;
...
public:
  explicit cirtoNAnoder(const ActionOptions&);
// active methods:
...
PLUMED_REGISTER_ACTION(cirtoNAnoder,"CIRTONANODER")

void cirtoNAnoder::registerKeywords(Keywords& keys) {
...
void cirtoNAnoder::completeBase(char type, AtomNumber c1, vector<AtomNumber> *atoms, AtomNumber *next){
  switch(type){
    case('A'):{
      (*atoms).resize(10);
      int start=c1.index();
...
cirtoNAnoder::cirtoNAnoder(const ActionOptions&ao):
...
void cirtoNAnoder::fit(char type, vector<VectorGeneric<3>> atoms, VectorGeneric<3> *fit1, VectorGeneric<3> *fit2, VectorGeneric<3> *fit3) {
...
double cirtoNAnoder::Determinant3x3(vector< VectorGeneric<3> > Matrix){
...
void cirtoNAnoder::EigenValsAndVectorsSymmetric(vector<VectorGeneric<3>> Matrix, VectorGeneric<3>* EigenValues, vector<VectorGeneric<3>>* EigenVectors){
...
double cirtoNAnoder::Norm(VectorGeneric<3> vec){
  double norm = vec.modulo();
  return norm;
}

VectorGeneric<3> cirtoNAnoder::Normalize(VectorGeneric<3> vec){
  double norm = vec.modulo();
  VectorGeneric<3> normalized = vec/norm;
  return normalized;
}

VectorGeneric<3> cirtoNAnoder::AxisRotation(VectorGeneric<3> v, VectorGeneric<3> k, double theta){
  VectorGeneric<3> vrot = v*cos(theta) + crossProduct(k,v)*sin(theta) + k*dotProduct(k,v)*(1-cos(theta));
  return vrot;
}

vector<VectorGeneric<3>> cirtoNAnoder::Skew(VectorGeneric<3> phi){
...
vector<VectorGeneric<3>> cirtoNAnoder::RotationMatrix(vector<VectorGeneric<3>> frame1, vector<VectorGeneric<3>> frame2){
...
VectorGeneric<3> cirtoNAnoder::RotationAxis(vector<VectorGeneric<3>> Q){
...
double cirtoNAnoder::RotationAngle(vector<VectorGeneric<3>> Q){
...
VectorGeneric<3> cirtoNAnoder::RotationVector(vector<VectorGeneric<3>> Q){
...
vector<VectorGeneric<3>> cirtoNAnoder::AxisAngleToMatrix(VectorGeneric<3> RotationVector){
  vector<VectorGeneric<3>> Q(3, Vector(0,0,0));
...
vector<VectorGeneric<3>> cirtoNAnoder::midFrame(vector<VectorGeneric<3>> frame1, VectorGeneric<3> RotationVector){
...
vector<VectorGeneric<3>> cirtoNAnoder::BaseFrame(VectorGeneric<3> glycosidic, VectorGeneric<3> internal){
...
VectorGeneric<3> cirtoNAnoder::RefPoint(VectorGeneric<3> glycosidic, VectorGeneric<3> normal, VectorGeneric<3> nitrogen){
...
void cirtocirtoNAnoderNA::calculate(){
  int i, j, l, k, n, N, M, x;

  switch(circirtoNAnodertoNA::INPUTMODE){
     ...
     // output
      ...
      for(step=0;step<nsteps;step++){      
       ...
       //now the derivatives of the parameters of this step are defined for all atoms
        for(N=0;N<natoms;N++){//atoms before the step give derivative 0
